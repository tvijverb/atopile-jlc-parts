...
pub async fn part_request(
    ...
    State(AppState{ ref resistor_df, ..}): State<AppState>,
    Json(payload): Json<ResistorRequest>
) -> (StatusCode, Response) {

    // validate the request
    if payload.tolerance_percentage.is_some() && payload.tolerance_percentage < Some(0.0) || payload.tolerance_percentage > Some(100.0) {
        return (StatusCode::BAD_REQUEST, Json(NoPartFound {code: 400, message: "Tolerance percentage must be between 0 and 100".to_string() } ).into_response());
    }
    ...
    if payload.absolute_tolerance.is_some() && payload.tolerance_percentage.is_some() {
        return (StatusCode::BAD_REQUEST, Json(NoPartFound {code: 400, message: "Either absolute_tolerance or tolerance_percentage must be set, not both".to_string() } ).into_response());
    }
    if payload.absolute_tolerance.is_none() && payload.tolerance_percentage.is_none() {
        return (StatusCode::BAD_REQUEST, Json(NoPartFound {code: 400, message: "Either absolute_tolerance or tolerance_percentage must be set, not neither".to_string() } ).into_response());
    }
    ...
    if payload.absolute_tolerance.is_some() && payload.absolute_tolerance_unit.is_none() {
        return (StatusCode::BAD_REQUEST, Json(NoPartFound {code: 400, message: "If absolute_tolerance is set, absolute_tolerance_unit must also be set".to_string() } ).into_response());
    }

    // all is well, let's find the part
    let part_dataframe_option = find_resistor(resistor_df.clone(), payload);

    if part_dataframe_option.is_none() {
        return (StatusCode::NOT_FOUND, Json(NoPartFound {code: 404, message: "No resistor found".to_string() } ).into_response());
    }
