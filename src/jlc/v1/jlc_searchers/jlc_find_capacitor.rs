...
pub fn find_capacitor(
    ...
    let mut components_df = components_df
        .filter(col("category_id").eq(lit(27)))
        .collect()
        .unwrap()
        .lazy();

    // value conversion
    let (farad_multiplier, _farad_string) = match request.value.unit.as_str() {
        "pF" | "picofarad" => (1.0, "pF"),
        "nF" | "nanofarad" => (1e3, "nF"),
    ...
    let components_df_eq = components_df
        .clone()
        .filter(col("capacitance").eq(lit(farad_value)));
    ...
    let components_df_range = components_df.filter(
        col("capacitance")
            .gt(lit(farad_min))
            .and(col("capacitance").lt(lit(farad_max))),
    );
