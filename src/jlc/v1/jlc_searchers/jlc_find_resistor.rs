...
pub fn find_resistor(
    ...
    let (ohm_value, ohm_max, ohm_min, _ohm_multiplier) = match ohm_string.as_str() {
        "pΩ" | "picoohm" => (
            ...
            request.value.nominal * 1e-12 * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * 1e-12 * (1.0 - request.value.tolerance_pct / 100.0),
            1e-12,
        ),
        "nΩ" | "nanoohm" => (
            request.value.nominal * 1e-9,
            request.value.nominal * 1e-9 * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * 1e-9 * (1.0 - request.value.tolerance_pct / 100.0),
            1e-9,
        ),
        "μΩ" | "uΩ" | "microohm" => (
            request.value.nominal * 1e-6,
            request.value.nominal * 1e-6 * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * 1e-6 * (1.0 - request.value.tolerance_pct / 100.0),
            1e-6,
        ),
        "mΩ" | "milliohm" => (
            request.value.nominal * 1e-3,
            request.value.nominal * 1e-3 * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * 1e-3 * (1.0 - request.value.tolerance_pct / 100.0),
            1e-3,
        ),
        "kΩ" | "kiloohm" => (
            request.value.nominal * 1e3,
            request.value.nominal * 1e3 * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * 1e3 * (1.0 - request.value.tolerance_pct / 100.0),
            1e3,
        ),
        "MΩ" | "megaohm" => (
            request.value.nominal * 1e6,
            request.value.nominal * 1e6 * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * 1e6 * (1.0 - request.value.tolerance_pct / 100.0),
            1e6,
        ),
        "Ω" | "ohm" => (
            request.value.nominal,
            request.value.nominal * (1.0 + request.value.tolerance_pct / 100.0),
            request.value.nominal * (1.0 - request.value.tolerance_pct / 100.0),
            1.0,
        ),
        _ => return None,
    ...
    let components_df_eq = components_df
        .clone()
        .filter(col("resistance").eq(lit(ohm_value)));
    ...
    let components_df_range: LazyFrame = components_df.filter(
        col("resistance")
            .gt(lit(ohm_min))
            .and(col("resistance").lt(lit(ohm_max))),
    );
